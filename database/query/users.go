...
const QueryCreateUser = `insert into users (created_at,user_name, password, status, user_id, email) values (?,?,?,?,?,?)`
...
const QueryLogin = `select id, user_name, user_id, password, status, role_id, email from users where user_name = ?`
